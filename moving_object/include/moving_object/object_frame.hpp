...
{
public:
  MovingObjectFrame(const builtin_interfaces::msg::Time& stamp, const std::string& frame_id,
                    const rclcpp::Node::SharedPtr node, const std::shared_ptr<Param> params);
   ...
   */
  void addVector(const std::vector<DetectionObjectInBox>& vector);
   ...
   */
  void addVector(const std::vector<TrackingObjectInBox>& vector);
   ...
   *         Object information is the one got from object module, such as Detection Vector,
   * Tracking Vector and
   *         Localization Vector.
   ...
   *  @param[in] roi Region of interest within an image, which is used to identify the different
   * objects in an image.
...
private:
  ...
  /**< @brief transform frame id which is archived from object topics and is generated for moving
   * topics.*/
  ...
  /**< @brief vectors storing object info(detection, tracking and localization, got from object
   * topics).*/
  ...
  rclcpp::Publisher<moving_object_msgs::msg::MovingObjectsInFrame>::SharedPtr
      moving_objects_pub_;                   /**<@brief ros publisher for moving object topic.*/
  std::vector<MovingObject> moving_objects_; /**<@brief vector storing the moving objects.*/
